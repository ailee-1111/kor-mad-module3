...
애플리케이션의 상세 페이지를 표시하려면 `DETAILS` 를 클릭합니다. 그런 다음 `PARAMETER` 탭으로 전환하고 `EDIT` 을 클릭합니다.
...
image::argocd-sync2.png[argocd-sync2]
...
image::app-topology2.png[app-topology2] 
