...
link:https://ordersfrontend-retail-%USERID%.%SUBDOMAIN%[GLOBEX web page^]에서 `Customers` 메뉴를 클릭합니다.

페이지가 정상적으로 표시되지 않습니다. 현재 서비스가 현대화된 customers 앱을 포인터로 바라보고 있지 않기 때문입니다.

image::old-page.png[old]
